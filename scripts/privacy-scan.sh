...
if find . -type f -not -path './.git' -not -path './.git/*' -not -path './node_modules/*' | grep -Eiq "\\.${blocked}"; then
...
if rg -n --hidden -g '!.git' -g '!.git/**' -g '!node_modules/**' -g '!scripts/privacy-scan.sh' -e "$patterns" .; then
